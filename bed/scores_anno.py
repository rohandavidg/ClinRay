...
"""
This script annotates a given bed file with score from the overlap
...
def configure_logger(value):
    ...
    setting up logging
    """
    logger = logging.getLogger('scores_Add')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(time.strftime("scores_Add" + str(value) + "-%Y%m%d.log"))
    ...
    try:
        ...
        with open(bed_file, 'r') as fh:
            try:
                ...
                logger.debug("unable to determine format of bed file {0}".format(bed_file))
